...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const API_URL =
    "https://leonidgr-projects-production.up.railway.app/projects";
  let allProjects = [];
            ...
            <div class="modal-section">
                ...
                <div class="modal-tech-stack">
                    ${project.technologies
                      .map(
                        (tech) => `<span class="modal-tech-item">${tech}</span>`
                      )
                      .join("")}
                </div>
            </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (contactSection) {
    const animatedElements = contactSection.querySelectorAll(
      ".contact-text h3, .contact-text p, .contact-method, .contact-form"
    );
...
function initCodeParticles() {
  ...
  function createRandomElement() {
    if (document.querySelectorAll(".code-particle").length > 25) return; // Limite le nombre d'éléments

    const element = document.createElement("div");
    element.classList.add("code-particle");
    ...
    if (typeRandom < 0.3) {
      ...
      content = technologies[Math.floor(Math.random() * technologies.length)];
